...
app.post("/api", async (req, res) => {
  ...
  try {
    const personExist = await Person.find({ name: name });
...
app.get("/api/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `Error finding ${personName}` });
  }
});
...
app.patch("/api/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `Error updating ${personName.name}` });
  }
});
...
app.delete("/api/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `Error deleting ${personName.name}` });
  }
});
